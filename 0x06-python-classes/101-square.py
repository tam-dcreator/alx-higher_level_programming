...
class Square:
    ...
    def __str__(self):
        """Special function that implements the default response when print is
        called on the class

        Returns:
           A string representation of the square using '#'
        """
        output_value = ""
        if self.__size == 0:
            pass
            ...
            for i in range(self.__size):
                output_value += " " * self.__position[0] + "#" * self.__size
                if i != self.__size - 1:
                    output_value += "\n"
        return output_value
